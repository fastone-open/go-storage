...
[![Build Status](https://git.fastonetech.com/fastone/go-storage/workflows/Unit%20Test/badge.svg?branch=master)](https://git.fastonetech.com/fastone/go-storage/actions?query=workflow%3A%22Unit+Test%22)
...
更多示例可以在 [go-storage-example](https://git.fastonetech.com/fastone/go-storage-example) 找到。
...
更多关于服务的想法可以在 [Service Integration Tracking](https://git.fastonetech.com/fastone/go-storage/issues/536) 找到。
...
- `id`: 服务中的唯一键
- `name`: 服务工作目录的相对路径
- `mode`: 对象的模式可以由以下几种进行组合：`read`, `dir`, `part` 以及 [更多](https://git.fastonetech.com/fastone/go-storage/blob/master/types/object.go#L11) 
...
自我维护的代码生成器 [定义](https://git.fastonetech.com/fastone/go-storage/tree/master/cmd/definitions) 有助于生成我们所有的 API、pairs 和元数据。
